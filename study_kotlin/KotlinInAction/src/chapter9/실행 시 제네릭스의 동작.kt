...
/** 단!! 자바 코드에서는 reified 타입 파라미터를 사용하는 inline 함수를 호출할 수 없다는 점을 기억하라.
 * 자바에서는 코틀린 인라인 함수를 다른 보통 함수처럼 호출한다. 그런 경우 인라인 함수를 호출해도 실제로 인라이닝이 되지 않는다.
 * 실체화한 타입 파라미터가 있는 함수의 경우 타입 인자 값을 바이트코드에 넣기 위해 일반 함수보다 더 많은 작업이 필요하고,
 * 항상 인라이닝 돼야만 한다. 따라서 실체화한 타입 파라미터가 있는 인라이닝 함수를 일반 함수처럼 자바에서 호출할 수 없다.
 */
...
//}
//==> startActivity<MainActivity>() // 이런식으로 호출

/** 실체화한 타입 파라미터는 유용하지만, 제약이 있다.
 ...
 * 3. 실체화한 타입 파라미터를 요구하는 함수를 호출하면서 실체화하지 않은
 * 타입 파라미터로 받은 타입을 타입 인자로 넘기기
